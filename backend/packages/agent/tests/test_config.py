...
def test_push_to_hub_picks_the_next_version(tmp_path):
    ...
    assert baseline.push_to_hub(repo, notes="first") == "1.3.0"  # nothing published yet: its own version
    assert baseline.push_to_hub(repo) == "1.3.1"
    assert baseline.push_to_hub(repo, bump="minor", notes="new copy") == "1.4.0"
    assert baseline.push_to_hub(repo, version="1.4.2") == "1.4.2"
    assert published_versions(tmp_path / "hub") == ["1.3.0", "1.3.1", "1.4.0", "1.4.2"]

    base = open_store(repo)
    note = release_of(base, "1.4.0")
    assert note["notes"] == "new copy" and note["via"] == "push_to_hub" and note["based_on"] == "1.3.0"
    assert json.loads(base.sub("1.4.0").read("config.json"))["version"] == "1.4.0"
...
def test_pull_edit_push_round_trip(tmp_path):
    ...
    assert edited.push_to_hub(repo, bump="minor") == "1.4.0"
    ...
    assert published_versions(tmp_path / "hub") == ["1.3.0", "1.4.0"]
...
def test_cli_push(tmp_path, capsys):
    ...
    assert "pushed 1.4.0 (from 1.3.0)" in capsys.readouterr().out
    assert main(["versions", repo]) == 0
    assert capsys.readouterr().out.split() == ["1.3.0", "1.4.0"]
