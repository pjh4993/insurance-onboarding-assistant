...
def edited_draft(client, version="1.3.0"):
...
def test_status_and_versions(operator_client):
    ...
    assert status["live"]["version"] == "1.3.0" and status["next"] == "1.3.0"
    ...
    detail = operator_client.get("/api/operator/config/versions/1.3.0", headers=OPERATOR).json()
...
def test_publish_bumps_and_restart_rolls_the_backend(operator_client, ecs):
    ...
    assert release["published_by"] == "operator-demo" and release["based_on"] == "1.3.0"
    assert release["notes"] == "friendlier wording" and release["via"] == "operator-console"

    # the same draft again is the next patch; the backend keeps running 1.3.0 until it restarts
    assert (
        operator_client.post("/api/operator/config/versions", headers=OPERATOR, json={"files": files}).json()["version"]
        == "1.4.1"
    )
    status = operator_client.get("/api/operator/config", headers=OPERATOR).json()
    assert status["live"]["version"] == "1.3.0" and status["next"] == "1.4.1" and status["restart_needed"]
...
def test_without_a_config_repo_the_console_only_reads(settings, external, llm):
    ...
    with TestClient(app) as client:
        ...
        files = client.get("/api/operator/config/versions/1.3.0", headers=OPERATOR).json()["files"]
