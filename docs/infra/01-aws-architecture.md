...
| **Route 53 + ACM** | the zone `onboardassist.click` (registered in Route 53, which created the hosted zone) holds a customer, an agent and a docs host per environment: develop `dev.app.`, `dev.agent.` and `dev.docs.onboardassist.click`, prod `app.`, `agent.` and `docs.onboardassist.click`; one certificate covers all three. The apex serves nothing. Terraform issues a DNS-validated ACM certificate and an alias record to the ALB | HTTPS and Cognito both need a domain. Public ACM certificates are free |
...
| Domain | `dev.app.` (customers), `dev.agent.` (agents, Cognito on every path) and `dev.docs.onboardassist.click`: HTTPS, one ACM certificate | `app.`, `agent.` and `docs.onboardassist.click`, set up the same way when prod is first applied |
